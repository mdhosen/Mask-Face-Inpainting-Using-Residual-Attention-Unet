...
class ResAttnUnet(tf.keras.models.Model):
    """
    ...
    def load_model(model_path, configuration=None):
        ...
        @return: ResAttnUnet
        """
        with CustomObjectScope({'ssim_loss': ResAttnUnet.ssim_loss, 'ssim_l1_loss': ResAttnUnet.ssim_l1_loss}):
            model = tf.keras.models.load_model(model_path)
        return ResAttnUnet(model, configuration)
    ...
    def build_model(architecture: UNet, input_size: Tuple[int, int, int], filters: Optional[Tuple] = None,
        ...
        @return: ResAttnUnet
        """
        return ResAttnUnet(architecture.build_model(input_size, filters, kernels), configuration)
    ...
    def train(self, epochs=20, batch_size=20, loss_function='mse', learning_rate=1e-4,
        ...
        train_dataset = ResAttnUnet.tf_dataset(train_x, train_y, batch_size, predict_difference)
        valid_dataset = ResAttnUnet.tf_dataset(valid_x, valid_y, batch_size, predict_difference, train=False)
        test_dataset = ResAttnUnet.tf_dataset(test_x, test_y, batch_size, predict_difference, train=False)

        # select loss
        if loss_function == 'ssim_loss':
            loss = ResAttnUnet.ssim_loss
        elif loss_function == 'ssim_l1_loss':
            loss = ResAttnUnet.ssim_l1_loss
        else:
            loss = loss_function
        ...
        callbacks = [
            ModelCheckpoint(
                f'models/model_epochs-{epochs}_batch-{batch_size}_loss-{loss_function}_{ResAttnUnet.get_datetime_string()}.h5'),
    ...
    def _test_results(self, test_x, test_y, predict_difference: bool):
        ...
        if self.configuration is None:
            result_dir = f'data/results/{ResAttnUnet.get_datetime_string()}/'
        else:
            result_dir = os.path.join(self.configuration.get('test_results_dir'), ResAttnUnet.get_datetime_string())
        os.makedirs(result_dir, exist_ok=True)

        for i, (x, y) in enumerate(zip(test_x, test_y)):
            x = ResAttnUnet.read_image(x)
            y = ResAttnUnet.read_image(y)
    ...
    def load_train_data(self, split=0.2):
        ...
        return ResAttnUnet.load_data(os.path.join(train_dir, 'inputs'), os.path.join(train_dir, 'outputs'), split, limit)
    ...
    def load_test_data(self):
        ...
        return ResAttnUnet.load_data(os.path.join(test_dir, 'inputs'), os.path.join(test_dir, 'outputs'), None, limit)
    ...
    def tf_parse(x, y):
        ...
        """
        def _parse(x, y):
            x = ResAttnUnet.read_image(x.decode())
            y = ResAttnUnet.read_image(y.decode())
            return x, y
    ...
    def tf_dataset(x, y, batch=8, predict_difference: bool = False, train: bool = True):
        ...
        dataset = dataset.map(ResAttnUnet.tf_parse)
